...
import {
  ...
  CustomerPermission,
  UpdateCustomerPermissionDto,
} from '@mdtx/database';
import { CustomerPermissionService } from './customer-permission.service';

const R = RestRouteBuilder.get('CustomerPermission');
...
export class CustomerPermissionController {
  ...
  @R.SaveOne()
  save(@R.Body() body: CreateCustomerPermissionDto) {
  ...
  updateOneById(
    @R.ParamID() id: number,
    @R.Body() body: UpdateCustomerPermissionDto
  ) {
    return this.service.updateOneById(id, body);
  }

  @R.AddRelation()
  addRelationToCustomerPermission(
    @R.Param() param: RelationDto<CustomerPermission>
  ) {
    return this.service.addRelation(param);
  }

  @R.RemoveRelation()
  removeRelation(@R.Param() param: RelationDto<CustomerPermission>) {
    return this.service.removeRelation(param);
  }

  @R.SetRelation()
  setRelation(@R.Param() param: RelationDto<CustomerPermission>) {
    return this.service.setRelation(param);
  }

  @R.UnsetRelation()
  unsetRelation(@R.Param() param: UnsetRelationDto<CustomerPermission>) {
