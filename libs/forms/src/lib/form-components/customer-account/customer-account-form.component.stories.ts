...
const meta: Meta<CustomerAccountFormComponent> = {
  component: CustomerAccountFormComponent,
  title: 'CustomerAccountFormComponent',
  decorators: [
    applicationConfig({
      providers: [
        provideAnimations(),
...
export default meta;
type Story = StoryObj<CustomerAccountFormComponent>;
...
export const Heading: Story = {
  play: async ({ canvasElement }) => {
    ...
    const submitForm = async () => {
      await type(name, 'customer-account name');
