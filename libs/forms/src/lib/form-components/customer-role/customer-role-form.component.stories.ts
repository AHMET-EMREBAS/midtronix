...
import { CustomerRoleFormComponent } from './customer-role-form.component';
...
const meta: Meta<CustomerRoleFormComponent> = {
  component: CustomerRoleFormComponent,
  title: 'CustomerRoleFormComponent',
  decorators: [
    applicationConfig({
      providers: [
        provideAnimations(),
...
export default meta;
type Story = StoryObj<CustomerRoleFormComponent>;
...
export const Heading: Story = {
  play: async ({ canvasElement }) => {
    ...
    const submitForm = async () => {
      await type(name, 'customer-role name');
